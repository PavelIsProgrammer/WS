...
class EmailCodeViewModel(
    ...
    private val _sendCodeToEmail: MutableStateFlow<DomainResult<MessageDomain>> =
        MutableStateFlow(DomainResult.Loading(LoadingState.INITIAL))
    val sendCodeToEmail = _sendCodeToEmail.asStateFlow()

    private val _token: MutableStateFlow<DomainResult<TokenDomain>> =
        MutableStateFlow(DomainResult.Loading(LoadingState.INITIAL))
    val token = _token.asStateFlow()

    private val _saveToken: MutableStateFlow<DomainResult<Unit>> =
        MutableStateFlow(DomainResult.Loading(LoadingState.INITIAL))
